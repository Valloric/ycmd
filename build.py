...
NO_DYNAMIC_PYTHON_ERROR = (
  'ERROR: found static Python library ({library}) but no dynamic. You must use '
  'a Python compiled with the {flag} flag. If using pyenv, you need to run the '
  'command:\n'
...
def FindPythonLibrariesOnLinux():
  ...
  # Python 3 has an 'm' suffix on Linux, for instance libpython3.3m.so.
  if PY_MAJOR == 3:
    python_name += 'm'

  python_library_root = '{0}/lib/lib{1}'.format( python_prefix, python_name )
  python_include = '{0}/include/{1}'.format( python_prefix, python_name )
...
def FindPythonLibrariesOnMac():
  ...
  # Python 3 has an 'm' suffix on OS X, for instance libpython3.3m.dylib.
  if PY_MAJOR == 3:
    python_name += 'm'
