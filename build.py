...
def GetPythonLibraryDirectory():
  # Get the directory containing standard Python library modules.
  modules_dir = sysconfig.get_python_lib( standard_lib = True )
  # On Windows, the Python library is alongside the Python modules in the
  # 'libs' directory.
  if OnWindows():
    return p.join( p.dirname( modules_dir ), 'libs' )
  # On POSIX platforms, the Python library is always found in the parent
  # directory of the Python modules or one of its subdirectories.
  return p.dirname( modules_dir )


def FindPythonLibraries():
  include_dir = sysconfig.get_config_var( 'INCLUDEPY' )
  library_dir = GetPythonLibraryDirectory()
  ...
  # We search the Python libraries through the library folder and its subfolders
  # because on some Linux distributions, they are not at the root of the library
