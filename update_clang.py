...
def BundleAndUpload( args, temp_dir, output_dir, os_name, download_data,
  ...
  clangd_package = download_data[ 'clangd_package' ][ 'name' ].format(
    os_name = os_name,
    llvm_version = args.version )
  libclang_package = download_data[ 'libclang_package' ][ 'name' ].format(
    os_name = os_name,
    llvm_version = args.version )
  download_url = download_data[ 'url' ].format( llvm_version = args.version,
                                                llvm_package = llvm_package )

  libclang_package_file = os.path.join( output_dir, libclang_package )
  clangd_package_file = os.path.join( output_dir, clangd_package )
  ...
  if 'run_after_extract' in download_data[ 'libclang_package' ]:
    extra_cmd = download_data[ 'libclang_package' ][ 'run_after_extract' ]
    extra_cmd( package_dir, args.version )

  for package_name, package_file in zip(
      [ 'libclang_package', 'clangd_package' ],
      [ libclang_package_file, clangd_package_file ] ):
  ...
  if not args.no_upload:
    for package in [ libclang_package_file, clangd_package_file ]:
