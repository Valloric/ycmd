...
PYBIND11_MODULE(ycm_core, m)
{
  ...
  class_< UnsavedFile >( m, "UnsavedFile" )
    .def( init<>() )
    .def_readwrite( "filename_", &UnsavedFile::filename_, return_value_policy::reference )
    .def_readwrite( "contents_", &UnsavedFile::contents_, return_value_policy::reference )
