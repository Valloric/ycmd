...
DocumentationData ClangCompleter::GetDocsForLocationInFile(
  ...
  // Do not reparse the translation unit as it has already been parsed
  // when getting the declaration's location, or it was just created and thus
  // got parsed, or it already existed but in that case its associated file is a
  // system header so it's not supposed to change.
