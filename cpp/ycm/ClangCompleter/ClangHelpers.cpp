...
std::string HandleChildDiags( std::vector< FixIt >& fixits,
                              CXDiagnostic diagnostic ) {
  ...
  std::string full_text = diag_text;
  ...
  for ( uint i = 0; i < num_child_diagnostics; ++i ) {
    ...
    // recurse
    full_text.append( HandleChildDiags( fixits, child_diag ) );
  }
  return full_text;
}
...
Diagnostic BuildDiagnostic( DiagnosticWrap diagnostic_wrap,
  ...
  diagnostic.long_formatted_text_ = HandleChildDiags( diagnostic.fixits_,
                                                      diagnostic_wrap.get() );
