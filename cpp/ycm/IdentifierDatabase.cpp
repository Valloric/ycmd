...
void IdentifierDatabase::ResultsForQueryAndType(
  ...
  google::dense_hash_set< const Candidate * > seen_candidates( candidate_repository_.NumStoredCandidates() );
...
google::dense_hash_set< const Candidate * > &IdentifierDatabase::GetCandidateSet(
  const std::string &filetype,
  const std::string &filepath ) {
  ...
  std::shared_ptr< google::dense_hash_set< const Candidate * > > &candidates =
    ( *path_to_candidates )[ filepath ];

  if ( !candidates ) {
    candidates.reset( new google::dense_hash_set< const Candidate * >() );
...
void IdentifierDatabase::AddIdentifiersNoLock(
  ...
  const std::string &filetype,
  const std::string &filepath ) {
  google::dense_hash_set< const Candidate *> &candidates =
