...
CLANG_THIRD_PARTY_DIR = os.path.join( DIR_OF_THIRD_PARTY, 'clang', 'lib' )
...
def LoadYcmCoreDependencies():
  for name in ListDirectory( CLANG_THIRD_PARTY_DIR ):
    if name.startswith( 'libclang' ):
      libclang_path = os.path.join( CLANG_THIRD_PARTY_DIR, name )
...
def GetYcmdResourceDir():
  resource_dir = os.path.join( CLANG_THIRD_PARTY_DIR, 'clang' )
  for version in ListDirectory( resource_dir ):
    return os.path.join( resource_dir, version )

  raise RuntimeError( 'Cannot find libclang resource directory.' )


YCMD_RESOURCE_DIR = GetYcmdResourceDir()
