...
def PathLeftSplit( path ):
  ...
  separator and tail is everything after. If there is no separator, head is the
  ...
  if offset == path_length:
    return drive + path, ''
  head = path[ offset + 1 : ].rstrip( separators )
  if offset == 0:
    return drive + path[ 0 ], head
  return drive + path[ : offset ], head
