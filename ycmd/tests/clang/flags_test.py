...
def GetSystemFlags_test():
  tests = [
    ...
    ( [ '-x', 'c++' ],              [ '-x', 'c++' ],              False ),
    ( [ '-xc' ],                    [ '-xc' ],                    False ),
    ( [ '-gcc-toolchain', '/foo' ], [ '-gcc-toolchain', '/foo' ], True ),
    ( [ '--gcc-toolchain=/foo' ],   [ '--gcc-toolchain=/foo' ],   True ),
    ( [ '-nostdinc' ],              [ '-nostdinc' ],              True ),
    ( [ '-nostdinc++' ],            [ '-nostdinc++' ],            True ),
    ( [ '--stdlib', 'libc++' ],     [ '--stdlib', 'libc++' ],     True ),
    ( [ '-stdlib=libc++' ],         [ '-stdlib=libc++' ],         True ),
    ( [ '--stdlib=libc++' ],        [ '--stdlib=libc++' ],        True ),
    ( [ '--sysroot', '/foo' ],      [ '--sysroot', '/foo' ],      True ),
    ( [ '--sysroot=/foo' ],         [ '--sysroot=/foo' ],         True ),
    ( [ '-target', 'foo' ],         [ '-target', 'foo' ],         True ),
    ( [ '--target=foo' ],           [ '--target=foo' ],           True ),
    ( [ '-foo', '-x', 'c', '-bar', '--target=/foo', '-wyz' ],
      [ '-x', 'c', '--target=/foo' ],                             False )
  ]
...
@patch( 'ycmd.completers.cpp.flags.REAL_CLANG_EXECUTABLE', None )
def AddSystemHeaderPaths_UseCppSystemHeadersIfNoExtension_test():
  class SafePopen( object ):
    def __init__( self, args, **kwargs ):
      utils.SafePopen( args, **kwargs ).communicate()
