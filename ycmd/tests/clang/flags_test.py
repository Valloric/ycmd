...
def GetSystemFlags( user_flags, expected_flags ):
...
def GetSystemFlags_test():
  ...
  for test in tests:
    yield GetSystemFlags, test[ 0 ], test[ 1 ]
...
def AddSystemHeaderPaths_RaiseErrorIfUnableToParseClang_test():
  ...
  assert_that(
    calling( flags._AddSystemHeaderPaths ).with_args( [], 'test' ),
    raises( RuntimeError,
