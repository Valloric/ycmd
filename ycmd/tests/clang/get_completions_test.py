...
def GetCompletions_ClientDataGivenToExtraConf_Cache_test( app ):
  ...
  # Parse the file and complete with flags from the client.
  event_notification = CombineRequest( request, {
    'event_name': 'FileReadyToParse',
    'extra_conf_data': {
      'flags': [ '-DSOME_MACRO' ]
    }
  } )
  app.post_json( '/event_notification', event_notification ).json
  ...
  # that case.
  completion_request = BuildRequest( **request )
