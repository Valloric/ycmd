...
def RunCompatibleWithCurrentCore( test, logger ):
  ...
  assert_that( code, equal_to( test[ 'exit_status' ] ) )

  if 'log_level' in test and 'log_message' in test:
    getattr( logger, test[ 'log_level' ] ).assert_called_once_with(
      test[ 'log_message' ] )
  else:
...
def CompatibleWithCurrentCore_Unexpected_test():
  RunCompatibleWithCurrentCore( {
    ...
    'exit_status': 3,
    'log_level': 'exception',
    'log_message': 'unexpected import error'
  } )
...
def CompatibleWithCurrentCore_Missing_test():
  ...
  for error in import_errors:
    yield RunCompatibleWithCurrentCore, {
      ...
      'exit_status': 4,
      'log_level': 'exception',
      'log_message': 'ycm_core library not detected; you need to compile it by '
                     'running the build.py script. See the documentation for '
                     'more details.'
    }
...
def CompatibleWithCurrentCore_Python2_test():
  ...
  for error in import_errors:
    yield RunCompatibleWithCurrentCore, {
      ...
      'exit_status': 5,
      'log_level': 'exception',
      'log_message': 'ycm_core library compiled for Python 2 '
...
def CompatibleWithCurrentCore_Python3_test():
  ...
  for error in import_errors:
    yield RunCompatibleWithCurrentCore, {
      ...
      'exit_status': 6,
      'log_level': 'exception',
      'log_message': 'ycm_core library compiled for Python 3 '
...
def CompatibleWithCurrentCore_Outdated_NoYcmCoreVersionMethod_test( *args ):
  RunCompatibleWithCurrentCore( {
    'exit_status': 7,
    'log_level': 'exception',
    'log_message': 'ycm_core library too old; PLEASE RECOMPILE by running the '
