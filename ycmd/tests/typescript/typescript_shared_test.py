...
class TypeScript_Shared_test( Typescript_Handlers_test ):
        ...
        'data': has_entries( {
          'completions': contains_inanyorder(
            self.CompletionEntryMatcher(
              'methodA', 'methodA (method) Foo.methodA(): void' ),
            self.CompletionEntryMatcher(
              'methodB', 'methodB (method) Foo.methodB(): void' ),
            self.CompletionEntryMatcher(
              'methodC', 'methodC (method) Foo.methodC(): void' ),
          )
        } )
      }
    } )
  ...
  def Subcommand_GoTo_Fail_test( self ):
    filepath = self._PathToTestFile( 'test.ts' )
    contents = ReadFile( filepath )
  ...
  def Subcommand_GetType_Basic_test( self ):
    filepath = self._PathToTestFile( 'test.ts' )
    contents = ReadFile( filepath )
  ...
  def Subcommand_GetType_HasNoType_test( self ):
    filepath = self._PathToTestFile( 'test.ts' )
    contents = ReadFile( filepath )
