...
def ClangdCompleter_FindClangBinary_WithUserOption_test():
  EXPECTED = 'test_path'
  user_options = { 'clangd_binary_path': EXPECTED }
  eq_( clangd_completer.FindClangdBinary( user_options ), EXPECTED )


def ClangdCompleter_FindClangBinary_WithoutUserOption_test():
  EXPECTED = 'third_party/clangd/output/bin/clangd'
  user_options = {}
  old_paths = os.environ[ 'PATH' ]
  os.environ[ 'PATH' ] = ''
  eq_( clangd_completer.FindClangdBinary( user_options ).endswith( EXPECTED ),
       True )
  os.environ[ 'PATH' ] = old_paths
