...
class CsharpSolutionCompleter( object ):
  ...
  def _FixIt( self, request_data ):
    ...
    if code_action_range:
      ...
      start_line = end_line = request_data[ 'line_num' ]
      start_column = 0
      end_column = len( request_data[ 'line_value' ] )

    request.update( {
      'SelectionStartColumn': start_column,
      'SelectionStartLine': start_line,
      'SelectionEndColumn': end_column,
    ...
    for i, code_action_name in enumerate( result[ 'CodeActions' ] ):
      chosen_range = {
        'start': { 'line_num': start_line, 'column_num': start_column },
        'end': { 'line_num': end_line, 'column_num': end_column } }
