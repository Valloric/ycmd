...
class TypeScriptCompleter( Completer ):
  ...
  def __init__( self, user_options ):
    ...
    # XXX(andrea): if we follow the path of tern_completer we can extract a
  ...
  def _RestartServer( self, request_data ):
    with self._server_lock:
      ...
      # while restarting the server. So if we restart the server and right
      # after ask completion in the buffer the server will timeout. So we
      # notify the server that we're working on the current buffer.
