...
class TypeScriptCompleter( Completer ):
  ...
  def __init__( self, user_options ):
    ...
    # FIXME: There's someting in the API that lists the trigger characters, but
    # i don't know how to get it from tsserver
  ...
  def ComputeSignaturesInner( self, request_data ):
    ...
    def MakeSignature( s ):
      label = _DisplayPartToString( s[ 'prefixDisplayParts' ] )
      parameters = []
      sep = _DisplayPartToString( s[ 'separatorDisplayParts' ] )
      for index, p in enumerate( s[ 'parameters' ] ):
        param = _DisplayPartToString( p[ 'displayParts' ] )
        start = len( label )
        end = start + len( param )
      ...
      label += _DisplayPartToString( s[ 'suffixDisplayParts' ] )
...
def _BuildCompletionExtraMenuAndDetailedInfo( request_data, entry ):
  signature = _DisplayPartToString( entry[ 'displayParts' ] )
...
def _DisplayPartToString( parts ):
