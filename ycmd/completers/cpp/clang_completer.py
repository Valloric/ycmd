...
def GetIncludeStatementValue( line, check_closing = True ):
  """Returns include statement value and boolean indicating whether include
  statement is quoted. If check_closing is True, the string is scanned for
  statement closing character (" or >) and substring between opening and closing
  characters is returned. The whole string after opening character and the
  column where the completion starts are returned otherwise."""
  ...
  if match:
    ...
    if not check_closing:
      ...
      if separator_char_pos != -1:
        start_codepoint = separator_char_pos + 1
        include_value = line[ include_start : start_codepoint ]
      else:
        start_codepoint = include_start
        include_value = ''
    else:
