...
def GetIncludeStatementValue( line, check_closing = True ):
  ...
  where:
  - |include_value| is the string from the starting quote or bracket of the
    include statement. If |check_closing| is True, the whole string between
    the quotes or brackets is returned. None if no include statement is found
    or if |check_closing| is True and there is no closing quote or bracket;
  - |quoted_include| is True if the statement is a quoted include, False
    otherwise;
  - |start_column| is the 1-based column where the completion starts. None if
