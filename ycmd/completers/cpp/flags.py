...
def PrepareFlagsForClang( flags,
  ...
  if add_extra_clang_flags:
    ...
    flags.append( '-resource-dir=' + YCMD_RESOURCE_DIR )
...
# as it's unable to find the framework headers without setting the sysroot if
# the Command Line Tools is not installed. So, we try to reproduce the logic
# used by Apple Clang to find the system headers by looking at the output of the
# command
#
#   clang++ -x c++ -E -v -
#
...
# navigation to those headers on that platform when the cursor is on include
# statements.
