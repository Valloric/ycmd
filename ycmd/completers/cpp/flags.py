...
YCMD_RESOURCE_DIR = os.path.abspath( os.path.join(
...
def _GetMacSysRoot():
  ...
  for sysroot in [ '/', MAC_PLATFORM_DIR ]:
...
# We can't rely on upstream libclang to find the system headers on macOS because
# it ignores the -resource-dir flag and use the current working directory to
# find the builtin includes instead, which fails since they are in a different
# place (in the clang_includes folder). Furthermore, it's unable to find the
# framework headers on macOS 10.14 without setting the sysroot. So, we try to
# reproduce the logic used by Apple Clang to find the system headers by looking
...
# This code probably differs from the latest version of Apple Clang but still
...
def _AddMacIncludePaths( flags ):
  ...
  if use_builtin_includes:
    flags.extend( [
      '-isystem', os.path.join( YCMD_RESOURCE_DIR, 'include' ) ] )
  if use_standard_system_includes:
    if toolchain:
      flags.extend( [
...
def _SpecialClangIncludes():
  return [ '-resource-dir=' + YCMD_RESOURCE_DIR ]
