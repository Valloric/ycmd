...
class JavaCompleter( language_server_completer.LanguageServerCompleter ):
  ...
  def OrganizeImports( self, request_data, project ):
    path = self._project_dir if project else request_data[ 'filepath' ]

    workspace_edit = self.GetCommandResponse( request_data,
