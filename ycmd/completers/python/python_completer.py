...
def _RefactoringToFixIt( refactoring ):
  ...
  # FIXME: refactorings can rename files (apparently). ycmd API doens't have any
  # interface for that, so we just ignore them.
  changes = refactoring.get_changed_files()
  chunks = []
  for filename, changed_file in changes.items():
    ...
    # Cache the offsets of all the newoines in the file. These are used to
    # calcualte the line/column values from the offsets retuned by the diff
    # scanner
...
def _OffsetToPosition( offset, filename, text, newlines ):
  ...
  # Invalid posistion - it's outside of the text. Just return the last
