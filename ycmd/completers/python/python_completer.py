...
class PythonCompleter( Completer ):
  ...
  def _SettingsForRequest( self, request_data ):
    ...
    settings = self._GetPythonSettings( module, client_data )
    self._settings_for_file[ filepath, client_data ] = settings
    return settings


  def _GetPythonSettings( self, module, client_data ):
    if not module:
      # We don't warn the user if no extra conf file is found.
      return {}

    if hasattr( module, 'PythonSettings' ):
      return module.PythonSettings( client_data = client_data )
    if hasattr( module, 'python_settings' ):
      return module.python_settings
    self._logger.debug( 'No python_settings variable or PythonSettings '
                        'function defined in %s', module.__file__ )
    return {
  ...
  def _GetSysPath( self, request_data, environment ):
    ...
    settings = {
      'interpreter_path': environment.executable,
      'sys_path': []
    }
