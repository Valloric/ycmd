...
class JediCompleter( Completer ):
  ...
  def ServerIsReady( self ):
    """
    Check is JediHTTP alive AND ready to serve requests.
    """
  ...
  def ServerIsRunning( self ):
    """
    Check is JediHTTP alive. That doesn't necessarily mean it's ready to serve
  ...
  def _GoToDefinition( self, request_data ):
    definitions = self._GetDefinitionsList( bytes( b'/gotodefinition' ),
                                            request_data )
    if not definitions:
  ...
  def _GoToDeclaration( self, request_data ):
    definitions = self._GetDefinitionsList( bytes( b'/gotoassignment' ),
                                            request_data )
    if not definitions:
  ...
  def _GoToReferences( self, request_data ):
    definitions = self._GetDefinitionsList( bytes( b'/usages' ), request_data )
    if not definitions:
