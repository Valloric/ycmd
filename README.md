...
of the following status codes if unsuccessful:

- 1: unexpected error while loading the library;
- 3: the `ycm_core` library is missing;
- 4: the `ycm_core` library is compiled for Python 3 but loaded in Python 2;
- 5: the `ycm_core` library is compiled for Python 2 but loaded in Python 3;
- 6: the version of the `ycm_core` library is outdated.
