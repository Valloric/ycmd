...
`package.json` file.
A `ls_config` object is a dictionary whose keys are "sections" and values are
pieces of settings (usually found in the `ls` object) corresponding to those
sections. This is even more underspecified and requires trial and error to
make it work. It is optional and only useful if you explicitly enable
...
def Settings( **kwargs ):
  if kwargs[ 'language' ] == 'java':
    return { 'ls': { 'java.rename.enabled' : False },
             # `ls_config` is not used for java...
             'ls_config': { 'section0': {} }
```
