...
In addition to the `PythonSettings` function, the Python settings can be set
through the `settings` dictionary under the `python` key. This is a more
convenient but less flexible way to configure the Python completer. The value of
the `python` key is defined as the dictionary returned by the `PythonSettings`
function.
